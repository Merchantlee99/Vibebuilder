...
| `codex-skill` | GPT-5.6 native-first skill router | 일반 작업은 모델에 맡기고, 스킬 하네스 작업만 명시적으로 route하며 전역 설치·backup·fixture/eval까지 제공하는 Codex Skill 패키지 |
...
- `codex-skill`은 v6보다 좁다. GPT-5.6의 native intent handling을 유지하면서 필요한 전문 스킬만 선택하고, 전역 설치와 회귀 검증까지 재현하는 reference package다.
...
- GPT-5.6용 얇은 전역 지시, 명시적 skill router, train/held-out eval, 안전한 전역 설치 흐름을 보고 싶으면 `codex-skill`이 맞다.
...
- GPT-5.6 Codex Skill router: [codex-skill/README.md](./codex-skill/README.md)
