...
class HarnessStructureTest(unittest.TestCase):
    ...
    def test_runtime_is_outside_codex(self) -> None:
        ...
        self.assertEqual(runtime.get("framework_version"), "v4")
        self.assertEqual(runtime.get("default_adoption_profile"), "creative-daily")
    ...
    def test_v4_simplicity_gate_template_passes(self) -> None:
        proc = self.run_cmd("scripts/harness/simplicity_gate.py", "--template", "--json")
        self.assertEqual(proc.returncode, 0, proc.stdout + proc.stderr)
        payload = json.loads(proc.stdout)
        self.assertTrue(payload["ok"])

    def test_v4_simplicity_gate_missing_optional_is_warning(self) -> None:
        proc = self.run_cmd(
    ...
    def test_v4_design_gate_template_passes(self) -> None:
        proc = self.run_cmd("scripts/harness/design_gate.py", "--template", "--json")
        self.assertEqual(proc.returncode, 0, proc.stdout + proc.stderr)
        payload = json.loads(proc.stdout)
        self.assertTrue(payload["ok"])

    def test_v4_design_gate_blocks_ui_scope_without_artifacts(self) -> None:
        proc = self.run_cmd(
