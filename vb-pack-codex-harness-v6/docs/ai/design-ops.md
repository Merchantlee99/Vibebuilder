...
Codex Harness v6 treats UI/UX work as intent-routed, spec-aware, evidence-backed product work, not only a code-generation task.
...
Use these templates under `docs/ai/current/` when the intent route or task profile marks UI evidence as required:
...
2. Create `Intent-Routing.json`, then `Task-Profile.json`.
