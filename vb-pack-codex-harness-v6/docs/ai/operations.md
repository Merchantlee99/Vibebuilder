...
- simplicity evidence exists when broad edits or refactors were made
- UI/UX artifacts exist when visual or interaction work was changed
...
This is optional for normal work. For high-risk reviews, v6 requires HMAC approval when `high_risk_requires_hmac_approval` is enabled and defaults to the policy-defined `HARNESS_REVIEW_SECRET` environment variable. It strengthens chain-of-trust only when the secret is held outside the Codex session. If Codex has access to the secret, treat it as additional audit evidence, not proof of human identity.
...
python3 scripts/harness/harness.py simplicity --artifact-dir docs/ai/current
python3 scripts/harness/harness.py design --artifact-dir docs/ai/current
```

## v6 Routing, Spec, Simplicity, And UI/UX Evidence

For normal+ work, create:

- `docs/ai/current/Intent-Routing.json`

For shared language or behavior work, create as routed:

- `docs/ai/current/Domain-Language.md`
- `docs/ai/current/Feature-Spec.md`
- `docs/ai/current/Req-Evidence-Map.md`
- `docs/ai/current/Spec-Review.md`

For broad implementation or refactor work, create:

- `docs/ai/current/Simplicity-Review.md`
...
- `docs/ai/current/Visual-Evidence.md`
