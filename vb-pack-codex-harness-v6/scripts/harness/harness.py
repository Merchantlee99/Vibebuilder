#!/usr/bin/env python3
"""Single entrypoint for Codex Harness v6 maintenance commands."""
...
def main() -> int:
    ...
    check.add_argument("--route-file", default="")
    check.add_argument("--domain-file", default="")
    check.add_argument("--spec-file", default="")
    check.add_argument("--req-map", default="")
    check.add_argument("--spec-review", default="")
    check.add_argument("--team-rule-path", default="harness/team/rule-proposals")
    ...
    check.add_argument("--strict-profile", choices=["intent-routed-spec-evidence", "implementation-first-evidence", "solo", "strict", "team", "production"])
    ...
    strict.add_argument("--profile", choices=["intent-routed-spec-evidence", "implementation-first-evidence", "solo", "strict", "team", "production"])
    ...
    intent_routing = sub.add_parser("intent-routing")
    intent_routing.add_argument("--route", default="docs/ai/current/Intent-Routing.json")
    intent_routing.add_argument("--template", action="store_true")
    intent_routing.add_argument("--json", action="store_true")
    intent_routing.add_argument("--warn-only", action="store_true")

    domain_language = sub.add_parser("domain-language")
    domain_language.add_argument("--path", default="docs/ai/current/Domain-Language.md")
    domain_language.add_argument("--term", action="append", default=[])
    domain_language.add_argument("--required", action="store_true")
    domain_language.add_argument("--template", action="store_true")
    domain_language.add_argument("--json", action="store_true")
    domain_language.add_argument("--warn-only", action="store_true")

    spec = sub.add_parser("spec")
    spec.add_argument("--spec", default="docs/ai/current/Feature-Spec.md")
    spec.add_argument("--require-layer", action="append", default=[])
    spec.add_argument("--template", action="store_true")
    spec.add_argument("--json", action="store_true")
    spec.add_argument("--warn-only", action="store_true")

    req_evidence = sub.add_parser("req-evidence")
    req_evidence.add_argument("--map", default="docs/ai/current/Req-Evidence-Map.md")
    req_evidence.add_argument("--spec", default="")
    req_evidence.add_argument("--allow-planned", action="store_true")
    req_evidence.add_argument("--template", action="store_true")
    req_evidence.add_argument("--json", action="store_true")
    req_evidence.add_argument("--warn-only", action="store_true")

    team_rule = sub.add_parser("team-rule")
    team_rule.add_argument("--path", default="harness/team/rule-proposals")
    team_rule.add_argument("--min-evidence", type=int, default=2)
    team_rule.add_argument("--required", action="store_true")
    team_rule.add_argument("--template", action="store_true")
    team_rule.add_argument("--json", action="store_true")
    team_rule.add_argument("--warn-only", action="store_true")

    rule_promotion = sub.add_parser("rule-promotion")
    rule_promotion.add_argument("--path", default="harness/team/rule-proposals")
    rule_promotion.add_argument("--allow-agents", action="store_true")
    rule_promotion.add_argument("--required", action="store_true")
    rule_promotion.add_argument("--template", action="store_true")
    rule_promotion.add_argument("--json", action="store_true")
    rule_promotion.add_argument("--warn-only", action="store_true")

    spec_drift = sub.add_parser("spec-drift")
    spec_drift.add_argument("--review", default="docs/ai/current/Spec-Review.md")
    spec_drift.add_argument("--required", action="store_true")
    spec_drift.add_argument("--template", action="store_true")
    spec_drift.add_argument("--json", action="store_true")
    spec_drift.add_argument("--warn-only", action="store_true")
    ...
    release.add_argument("--template", action="store_true")
    release.add_argument("--json", action="store_true")
    release.add_argument("--release-intent", choices=["none", "candidate", "publish"], default="none")
    release.add_argument("--version", default="")
    release.add_argument("--allow-dirty", action="store_true")
    release.add_argument("--allow-existing-tag", action="store_true")
    release.add_argument("--require-existing-tag", action="store_true")
    ...
    if args.command == "check":
        ...
        if args.route_file:
            command.extend(["--route-file", args.route_file])
        if args.domain_file:
            command.extend(["--domain-file", args.domain_file])
        if args.spec_file:
            command.extend(["--spec-file", args.spec_file])
        if args.req_map:
            command.extend(["--req-map", args.req_map])
        if args.spec_review:
            command.extend(["--spec-review", args.spec_review])
        if args.team_rule_path:
            command.extend(["--team-rule-path", args.team_rule_path])
        if args.review_file:
    ...
    if args.command == "ui-evidence":
        ...
        if args.warn_only:
            command.append("--warn-only")
        return run(command)
    if args.command == "intent-routing":
        command = ["scripts/harness/intent_router_gate.py", "--route", args.route]
        if args.template:
            command.append("--template")
        if args.json:
            command.append("--json")
        if args.warn_only:
            command.append("--warn-only")
        return run(command)
    if args.command == "domain-language":
        command = ["scripts/harness/domain_language_gate.py", "--path", args.path]
    ...
    if args.command == "spec":
        command = ["scripts/harness/spec_gate.py", "--spec", args.spec]
        for layer in args.require_layer:
            command.extend(["--require-layer", layer])
        if args.template:
            command.append("--template")
        if args.json:
            command.append("--json")
        if args.warn_only:
            command.append("--warn-only")
        return run(command)
    if args.command == "req-evidence":
        command = ["scripts/harness/req_evidence_gate.py", "--map", args.map]
        if args.spec:
            command.extend(["--spec", args.spec])
        if args.allow_planned:
            command.append("--allow-planned")
        if args.template:
            command.append("--template")
        if args.json:
            command.append("--json")
        if args.warn_only:
            command.append("--warn-only")
        return run(command)
    if args.command == "team-rule":
        command = ["scripts/harness/team_rule_mining_gate.py", "--path", args.path, "--min-evidence", str(args.min_evidence)]
        if args.required:
            command.append("--required")
        if args.template:
            command.append("--template")
        ...
        if args.warn_only:
            command.append("--warn-only")
        return run(command)
    if args.command == "rule-promotion":
        command = ["scripts/harness/rule_promotion_gate.py", "--path", args.path]
        if args.allow_agents:
            command.append("--allow-agents")
        if args.required:
            command.append("--required")
        if args.template:
            command.append("--template")
    ...
    if args.command == "spec-drift":
        command = ["scripts/harness/spec_drift_gate.py", "--review", args.review]
        if args.required:
            command.append("--required")
        if args.template:
            command.append("--template")
    ...
    if args.command == "release":
        command = ["scripts/harness/release_gate.py"]
        if args.template:
            command.append("--template")
        if args.json:
            command.append("--json")
        if args.release_intent:
            command.extend(["--release-intent", args.release_intent])
        if args.version:
            command.extend(["--version", args.version])
        if args.allow_dirty:
            command.append("--allow-dirty")
        if args.allow_existing_tag:
            command.append("--allow-existing-tag")
        if args.require_existing_tag:
            command.append("--require-existing-tag")
        return run(command)
