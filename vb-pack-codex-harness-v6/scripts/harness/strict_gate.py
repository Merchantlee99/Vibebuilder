#!/usr/bin/env python3
"""Audit Codex Harness v6 strict/high-risk readiness."""
...
def audit(args: argparse.Namespace) -> dict:
    ...
    profile = args.profile or runtime.get("default_adoption_profile", "intent-routed-spec-evidence")
    ...
    if runtime.get("review", {}).get("high_risk_requires_hmac_approval") is not True:
        errors.append("high-risk review must require HMAC approval in v6 strict policy")
    if runtime.get("review", {}).get("prepared_event_required") is not True:
        errors.append("prepared review event must be required in v6 strict policy")
...
def main() -> int:
    ...
    parser.add_argument("--profile", choices=["intent-routed-spec-evidence", "implementation-first-evidence", "solo", "strict", "team", "production"])
