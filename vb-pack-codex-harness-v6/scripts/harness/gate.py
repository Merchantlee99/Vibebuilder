#!/usr/bin/env python3
"""Deterministic gates for Codex Harness v6."""
...
from domain_language_gate import run_gate as run_domain_language_gate
from intent_router_gate import load_route, run_gate as run_intent_router_gate
from non_web_ui_evidence_gate import run_gate as run_non_web_ui_gate
from req_evidence_gate import run_gate as run_req_evidence_gate
from risk_classifier import classify_text
from rule_promotion_gate import run_gate as run_rule_promotion_gate
from runtime_evidence_gate import run_gate as run_runtime_gate
from spec_drift_gate import run_gate as run_spec_drift_gate
from spec_gate import run_gate as run_spec_gate
from strict_gate import audit as run_strict_audit
from task_profile_gate import load_profile, validate_profile
from team_rule_mining_gate import run_gate as run_team_rule_gate
...
REQUIRED_CONTROL_FILES = [
    ...
    "gstack/README.md",
    "gstack/workflows/intent-routed-implementation.md",
    "gstack/workflows/grill-with-docs-team-rules.md",
    "gstack/workflows/ui-evidence-review.md",
    "gstack/workflows/high-risk-strict.md",
    ...
    ".agents/skills/intent-routing/SKILL.md",
    ".agents/skills/grill-me/SKILL.md",
    ".agents/skills/grill-with-docs/SKILL.md",
    ".agents/skills/domain-language/SKILL.md",
    ".agents/skills/spec-authoring/SKILL.md",
    ".agents/skills/req-evidence/SKILL.md",
    ".agents/skills/team-rule-mining/SKILL.md",
    ...
    "templates/Intent-Routing.json",
    "templates/Domain-Language.md",
    "templates/Feature-Spec.md",
    "templates/ADR.md",
    "templates/Team-Rule-Proposal.md",
    "templates/Req-Evidence-Map.md",
    "templates/Spec-Review.md",
    "spec/00_constitution.md",
    "spec/README.md",
    "harness/routing/README.md",
    "harness/spec/README.md",
    "harness/team/README.md",
    ...
    "scripts/harness/release_gate.py",
    "scripts/harness/intent_router_gate.py",
    "scripts/harness/domain_language_gate.py",
    ...
    "scripts/harness/spec_drift_gate.py",
]
...
def preflight(_: argparse.Namespace) -> GateResult:
    ...
    if runtime.get("framework_version") != "v6":
        result.error("runtime framework_version must be v6.")
...
def route_path_for(args: argparse.Namespace) -> Path:
    if args.route_file:
        return ROOT / args.route_file
    return ROOT / args.artifact_dir / "Intent-Routing.json"


def v6_intent_gates(args: argparse.Namespace) -> GateResult:
    result = GateResult()
    if args.template:
        for prefix, outcome in [
            ("intent_router_gate", run_intent_router_gate(argparse.Namespace(route="", template=True, json=False, warn_only=False))),
            ("domain_language_gate", run_domain_language_gate(argparse.Namespace(path="", term=[], required=True, template=True, json=False, warn_only=False))),
            ("spec_gate", run_spec_gate(argparse.Namespace(spec="", require_layer=[], template=True, json=False, warn_only=False))),
            ("req_evidence_gate", run_req_evidence_gate(argparse.Namespace(map="", spec="", allow_planned=False, template=True, json=False, warn_only=False))),
            ("team_rule_mining_gate", run_team_rule_gate(argparse.Namespace(path="", min_evidence=2, required=True, template=True, json=False, warn_only=False))),
            ("rule_promotion_gate", run_rule_promotion_gate(argparse.Namespace(path="", allow_agents=False, required=False, template=True, json=False, warn_only=False))),
            ("spec_drift_gate", run_spec_drift_gate(argparse.Namespace(review="", required=True, template=True, json=False, warn_only=False))),
        ]:
            extend_tuple(result, outcome, prefix)
        return result

    if not tier_at_least(args.tier, "normal"):
        return result

    route_path = route_path_for(args)
    extend_tuple(result, run_intent_router_gate(argparse.Namespace(
        route=str(route_path.relative_to(ROOT)),
        template=False,
        json=False,
        warn_only=False,
    )), "intent_router_gate")
    route, load_errors = load_route(route_path)
    if load_errors:
        return result

    required_gates = route.get("required_gates", []) if isinstance(route.get("required_gates", []), list) else []
    spec_layers = route.get("spec_layers", []) if isinstance(route.get("spec_layers", []), list) else []
    if "domain_language_gate" in required_gates or route.get("needs_domain_language") is True:
        extend_tuple(result, run_domain_language_gate(argparse.Namespace(
            path=args.domain_file or f"{args.artifact_dir}/Domain-Language.md",
            term=[],
            required=True,
            template=False,
            json=False,
    ...
    if "spec_gate" in required_gates or route.get("needs_spec") is True:
        extend_tuple(result, run_spec_gate(argparse.Namespace(
            spec=args.spec_file or f"{args.artifact_dir}/Feature-Spec.md",
            require_layer=spec_layers,
            template=False,
            json=False,
            warn_only=False,
        )), "spec_gate")
    if "req_evidence_gate" in required_gates or route.get("governing_reqs"):
        extend_tuple(result, run_req_evidence_gate(argparse.Namespace(
            map=args.req_map or f"{args.artifact_dir}/Req-Evidence-Map.md",
            spec=args.spec_file or f"{args.artifact_dir}/Feature-Spec.md",
            allow_planned=False,
            template=False,
            json=False,
    ...
    if "team_rule_mining_gate" in required_gates or route.get("needs_team_rule_scan") is True:
        extend_tuple(result, run_team_rule_gate(argparse.Namespace(
            path=args.team_rule_path,
            min_evidence=2,
            required=True,
            template=False,
            json=False,
    ...
    if "rule_promotion_gate" in required_gates or route.get("team_rule_promotion_allowed") is True:
        extend_tuple(result, run_rule_promotion_gate(argparse.Namespace(
            path=args.team_rule_path,
            allow_agents=False,
            required=True,
            template=False,
    ...
    if "spec_drift_gate" in required_gates:
        extend_tuple(result, run_spec_drift_gate(argparse.Namespace(
            review=args.spec_review or f"{args.artifact_dir}/Spec-Review.md",
            required=True,
            template=False,
            json=False,
            warn_only=False,
...
def legacy_evidence_gates(args: argparse.Namespace) -> GateResult:
    result = GateResult()
    if args.template:
...
def main() -> int:
    ...
    parser.add_argument("--domain-file", default="")
    parser.add_argument("--spec-file", default="")
    parser.add_argument("--req-map", default="")
    parser.add_argument("--spec-review", default="")
    parser.add_argument("--team-rule-path", default="harness/team/rule-proposals")
    ...
    parser.add_argument("--strict-profile", choices=["intent-routed-spec-evidence", "implementation-first-evidence", "solo", "strict", "team", "production"])
