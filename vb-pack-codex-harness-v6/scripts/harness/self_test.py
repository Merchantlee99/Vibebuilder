#!/usr/bin/env python3
"""Structural self-test for Codex Harness v6."""
...
REQUIRED_FILES = [
    ...
    "docs/ai/v6-final-blueprint.md",
    "docs/ai/routing-operations.md",
...
def main() -> int:
    ...
    try:
        ...
        if runtime.get("framework_version") != "v6":
            errors.append("runtime framework_version must be v6")
        if runtime.get("default_adoption_profile") != "intent-routed-spec-evidence":
            errors.append("runtime default_adoption_profile must be intent-routed-spec-evidence")
        if runtime.get("review", {}).get("high_risk_requires_hmac_approval") is not True:
            errors.append("runtime high-risk review must require HMAC approval support")
        if runtime.get("review", {}).get("prepared_event_required") is not True:
            errors.append("runtime prepared_event_required must be true")
    except Exception as exc:
    ...
    for command in [
        ...
        [sys.executable, "scripts/harness/simplicity_gate.py", "--template", "--json"],
        [sys.executable, "scripts/harness/design_gate.py", "--template", "--json"],
        [sys.executable, "scripts/harness/strict_gate.py", "--template", "--json"],
        [sys.executable, "scripts/harness/task_profile_gate.py", "template", "--json"],
        [sys.executable, "scripts/harness/implementation_gate.py", "--template", "--json"],
        [sys.executable, "scripts/harness/ui_evidence_gate.py", "--template", "--json"],
        [sys.executable, "scripts/harness/runtime_evidence_gate.py", "--template", "--json"],
        [sys.executable, "scripts/harness/non_web_ui_evidence_gate.py", "--template", "--json"],
        [sys.executable, "scripts/harness/memory_guard.py", "audit", "--json"],
        [sys.executable, "scripts/harness/release_gate.py", "--template", "--json"],
        [sys.executable, "scripts/harness/intent_router_gate.py", "--template", "--json"],
        [sys.executable, "scripts/harness/domain_language_gate.py", "--template", "--json"],
        [sys.executable, "scripts/harness/spec_gate.py", "--template", "--json"],
        [sys.executable, "scripts/harness/req_evidence_gate.py", "--template", "--json"],
        [sys.executable, "scripts/harness/team_rule_mining_gate.py", "--template", "--json"],
        [sys.executable, "scripts/harness/rule_promotion_gate.py", "--template", "--json"],
        [sys.executable, "scripts/harness/spec_drift_gate.py", "--template", "--json"],
        ...
        [sys.executable, "scripts/harness/harness.py", "strict", "--template", "--json"],
        [sys.executable, "scripts/harness/harness.py", "simplicity", "--template", "--json"],
        [sys.executable, "scripts/harness/harness.py", "design", "--template", "--json"],
    ]:
