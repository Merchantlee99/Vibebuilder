#!/usr/bin/env python3
"""Run quick v6 harness benchmarks and optional baseline comparison."""
...
def main() -> int:
    ...
    if args.json:
        ...
        print(f"current total: {current['total_elapsed_ms']}ms")
        if baseline:
            print(f"baseline total: {baseline['total_elapsed_ms']}ms")
        for warning in result["comparison"].get("warnings", []):
            print(f"WARN: {warning}")
        print(output)
    has_failures = bool(current["failed"] or result["comparison"].get("warnings"))
