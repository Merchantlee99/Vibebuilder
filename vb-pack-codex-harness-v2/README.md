...
`score.py`는 실전 성과 점수가 아니라 readiness score입니다. 파일 구조, audit, gate, model policy, observability가 템플릿 기준을 만족하는지 확인합니다. 실제 운영 품질은 `ops_metrics.py`, review 결과, validation evidence, CI 실패율을 함께 봐야 합니다.
...
- hooks는 experimental이므로 템플릿 기본값은 off입니다. 실제 프로젝트에서는 `adopt_project.py --write --enable-hooks`로 명시적으로 켭니다.
