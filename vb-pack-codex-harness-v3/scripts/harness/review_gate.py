...
from common import ROOT, load_json, parse_key_value_lines, nonempty_section, utc_slug, event_id
...
def hmac_required(args: argparse.Namespace, fields: dict[str, str]) -> bool:
    if args.require_hmac:
        return True
    runtime = load_json(RUNTIME, {})
    if runtime.get("enforcement_mode") != "enforced":
        return False
    review_policy = runtime.get("review", {})
    tier = fields.get("Tier", "")
    if tier == "high-risk":
        return review_policy.get("high_risk_requires_hmac_approval") is True
    if tier == "normal":
        return review_policy.get("normal_requires_hmac_approval") is True
    return False
...
def finalize(args: argparse.Namespace) -> int:
    ...
    if hmac_required(args, fields) and not approval_verified:
        errors.append("HMAC review approval is required")
...
def main() -> int:
    ...
    fin.add_argument("--require-hmac", action="store_true")
