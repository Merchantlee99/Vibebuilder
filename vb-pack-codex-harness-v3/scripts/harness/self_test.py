#!/usr/bin/env python3
"""Structural self-test for Codex Harness v3."""
...
def main() -> int:
    ...
    try:
        ...
        if runtime.get("default_adoption_profile") != "strict":
            errors.append("runtime default_adoption_profile must be strict")
        if runtime.get("review", {}).get("high_risk_requires_hmac_approval") is not True:
            errors.append("high-risk HMAC approval must be enabled in v3 runtime policy")
    except Exception as exc:
