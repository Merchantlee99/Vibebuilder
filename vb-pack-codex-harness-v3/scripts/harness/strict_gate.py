#!/usr/bin/env python3
"""Audit Codex Harness v3 strict/high-trust readiness."""
...
def audit(args: argparse.Namespace) -> dict:
    ...
    profile = args.profile or runtime.get("default_adoption_profile", "strict")
    ...
    if profile_policy.get("hooks_required") and hooks_enabled is not True:
        errors.append(".codex/config.toml must set codex_hooks = true for strict profile")

    if runtime.get("framework_version") != "v3":
        errors.append("runtime framework_version must be v3")
    ...
    if runtime.get("review", {}).get("high_risk_requires_hmac_approval") is not True:
        errors.append("high-risk review must require HMAC approval in v3 strict policy")
    if runtime.get("review", {}).get("prepared_event_required") is not True:
        errors.append("prepared review event must be required in v3 strict policy")
...
def main() -> int:
    ...
    parser.add_argument("--profile", choices=["solo", "strict", "team", "production"])
