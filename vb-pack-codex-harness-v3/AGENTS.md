...
This repository is a Codex-only strict/high-trust harness template for natural-language product and software development work.
...
- Treat v3 as stricter than the daily v2 harness. Prefer explicit evidence over speed when the work is `normal+`.
...
- For `high-risk` work under enforced/strict mode, require HMAC review approval unless the user explicitly accepts the residual identity risk.
...
- Use `scripts/harness/strict_gate.py` before final completion for strict/team/production projects.
...
- Use `scripts/harness/strict_gate.py --template` to verify v3 strict readiness.
